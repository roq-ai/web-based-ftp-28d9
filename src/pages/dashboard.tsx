...
import { FtpUserListPage as ShowCaseList } from 'pages/ftp-users';
...
function DashboardPage() {
  ...
  const folderStat = useSWR<PaginatedInterface<FolderInterface>>(
    () => `/folders`,
    () => getFolders({ limit: 1, offset: 0 }),
  );

  const ftpUserStat = useSWR<PaginatedInterface<FtpUserInterface>>(
    () => `/ftp-users`,
    () => getFtpUsers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Folders',
      result: folderStat?.data?.totalCount,
      isLoading: folderStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Ftp Users',
      result: ftpUserStat?.data?.totalCount,
      isLoading: ftpUserStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`ftp-users`} passHref legacyBehavior>
                  <Button
