...
import { getFolderById } from 'apiSdk/folders';
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('folder', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/folders/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Path" text={data?.path} />
                ...
                <FormListItem
                  label="Last Modified"
                  text={
                    data?.last_modified ? format(parseISO(data?.last_modified as unknown as string), 'dd-MM-yyyy') : ''
                  }
                />
                ...
                {hasAccess('ftp_user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Ftp User"
                    text={
                      <Link as={NextLink} href={`/ftp-users/view/${data?.ftp_user?.id}`}>
                        {data?.ftp_user?.username}
                      </Link>
                    }
                  />
                )}
