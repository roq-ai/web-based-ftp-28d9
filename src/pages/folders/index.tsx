...
type ColumnType = ColumnDef<FolderInterface, unknown>;
...
export function FolderListPage(props: FolderListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getFolders({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'path.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FolderInterface>>(
    () => `/folders?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'size', header: 'size', accessorKey: 'size' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.last_modified ? format(parseISO(record?.last_modified as unknown as string), 'dd-MM-yyyy') : '',
    },
    hasAccess('ftp_user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/ftp-users/view/${record.ftp_user?.id}`}>
              {record.ftp_user?.username}
            </Link>
          ),
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/folders/view/${record.id}`} passHref legacyBehavior>
                <Button
